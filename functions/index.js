...
function TicketServer(interaction, questions, client) {
  interaction.followUp(`<@${interaction.user.id}> Ask your question, Support will answer you shortly
      `);
  const filter = (m) => {
    ...
    if (m.author.id === interaction.user.id) {
      ...
      if (!match) {
        ...
        try {
          ...
          let chan = await guild.channels.cache.find(
  ...
  collector.on("end", async (c) => {
    console.log("Time Ended. raffel or whitelist");
  });
}
